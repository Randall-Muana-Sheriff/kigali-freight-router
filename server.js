...
// Optional: Create a bot via @BotFather to receive instant incident updates on your phone
...
// Helper for automated dispatch warnings
...
// --- JWT AUTH SECURITY GATEWAY ---
app.post('/api/auth/login', (req, res) => {
  const { username } = req.body;
  if (username === 'Muana' || username.startsWith('sim_driver')) {
    const token = jwt.sign({ username }, JWT_SECRET, { expiresIn: '2h' });
    return res.json({ token });
  }
  return res.status(401).json({ error: 'Invalid dispatcher credentials' });
});
...
app.get('/api/routes', async (req, res) => {
  try {
    const result = await pool.query('SELECT id, driver_name AS "driverName", original_points_count AS "originalPointsCount", simplified_points_count AS "simplifiedPointsCount", space_saved_percentage AS "spaceSavedPercentage", ST_AsGeoJSON(geom_simplified) AS "geojsonSimplified" FROM routes ORDER BY id DESC');
...
app.post('/api/dispatch/matrix', verifyToken, async (req, res) => {
  ...
  try {
    ...
    const rankings = activeFleet.map((driver, index) => {
...
// Create dynamic polygon geofence with custom speed limit rule
...
app.get('/api/geofences', async (req, res) => {
  try {
    const result = await pool.query('SELECT id, name, speed_limit_kmh AS "speedLimitKmh", ST_AsGeoJSON(geom) as geojson FROM geofences ORDER BY id DESC');
  ...
  try {
    ...
    const insertQuery = `
      WITH raw_data AS (
        SELECT 
          $1::varchar AS d_name,
          ST_GeomFromText($2, 4326) AS g_orig,
          ST_SimplifyPreserveTopology(ST_GeomFromText($3, 4326), 0.0001) AS g_simp
      )
      INSERT INTO routes (driver_name, original_points_count, simplified_points_count, space_saved_percentage, geom_original, geom_simplified)
      SELECT 
        d_name,
        ST_NPoints(g_orig),
        ST_NPoints(g_simp),
        CONCAT(ROUND((1.0 - (ST_NPoints(g_simp)::float / ST_NPoints(g_orig)::float)) * 100), '%'),
        g_orig,
        g_simp
      FROM raw_data
      RETURNING original_points_count, simplified_points_count, space_saved_percentage;
    `;
...
// --- TELEMETRY & MULTI-RULE INFERENCE ENGINE ---
let liveFleetState = {};
...
io.on('connection', (socket) => {
  ...
  socket.on('driver:telemetry-push', async (data) => {
    ...
    // Simulate current velocity for standard telemetry streams
    ...
    try {
      // High performance GiST-backed containment lookup
      const boundaryCheck = await pool.query(`
        SELECT id, name, speed_limit_kmh AS "speedLimitKmh" FROM geofences 
      ...
      if (activeZone) {
        ...
        if (!ongoingViolation || ongoingViolation.zoneName !== activeZone.name || ongoingViolation.type !== violationType) {
          ...
          // Dispatch Telegram notification event
          dispatchExternalAlert(`🚨 *CRITICAL SAFETY INCIDENT* 🚨\n\n*Asset:* ${driverName}\n*Incident:* ${violationType}\n*Detail:* ${description}\n*Timestamp:* ${new Date(timestamp).toLocaleTimeString()}`);
        }
        ...
        console.error("❌ DATABASE ERROR:", dbErr); // This will show the full error details
      }
  });
});

server.listen(5000, () => {
  console.log('🚀 Secured Core Telemetry Routing Engine online on port 5000');
});